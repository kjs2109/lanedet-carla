...
class World(object):
    def __init__(self, carla_world, hud, args, log_writer, camera_folder):
    ...
    def restart(self):
        ...
        self.gnss_sensor = GnssSensor(self.player, self.log_writer)  #############################
        self.imu_sensor = IMUSensor(self.player, self.log_writer)    #####################################
        self.camera_manager = CameraManager(self.player, self.hud, self._gamma, self.camera_folder)  ##########
...
class GnssSensor(object):
    def __init__(self, parent_actor, log_writer):
        self.sensor = None
        self._parent = parent_actor
        self.lat = 0.0
        self.lon = 0.0
    ...
    def _on_gnss_event(weak_self, event):
        ...
        self.log_writer.writerow([time.time(), self._parent.id, self.lat, self.lon, None, None, None, None, None, None])
...
class IMUSensor(object):
    ...
    def _IMU_callback(weak_self, sensor_data):
        ...
        self.log_writer.writerow([time.time(), self._parent.id, None, None, self.accelerometer[0], self.accelerometer[1], self.accelerometer[2], self.gyroscope[0], self.gyroscope[1], self.gyroscope[2]])
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction, camera_folder):
        self.sensor = None
        self.surface = None
        self._parent = parent_actor
        self.hud = hud
        ...
        ##카메라 위치 설정 코드 (블랙박스 위치에 맞게 설정)


        self._camera_transforms = [
            (carla.Transform(carla.Location(x=1.0, y=0.0, z=1.2), carla.Rotation(pitch=0.0)), Attachment.Rigid)
        ]
        ...
        if self.recording:
            if not os.path.exists(self.camera_folder):
                try:
                    ...
                    print(f"Failed to create directory {self.camera_folder}: {e}")
                    return
            filename = f'{self.camera_folder}/%08d.png' % image.frame
...
## PNG 파일로 저장하겠다는 코드 추가 그리고 번호 순서를 매기는 코드 추가
...
def game_loop(args):
    ...
    try:
        ...
        if args.autopilot and not carla_world.get_settings().synchronous_mode:
            print("WARNING: You are currently in asynchronous mode and could "
                  "experience some issues with the traffic simulation")
